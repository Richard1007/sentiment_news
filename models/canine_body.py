...
def encode_data(dataset, tokenizer, max_seq_length=1024):
    l=[]
    body=dataset["Body"].to_list()
    for i in range(len(body)):
        if type(body[i])!="string":
            body[i]=str(body[i])
        l.append(body[i]) 
...
class DebertaDataset(Dataset):
    ...
    def __init__(self, dataframe, tokenizer, max_seq_length=1024):
        """
        Args:
